...
#import pickle as pickpick
#import joblib
#from joblib import load
#import dill

#loaded_model = pickpick.load(open("xgb_model.pkl", "rb"))
#loaded_model = load('xgb_model.joblib')
# with open("xgb_model.dill", "rb") as f:
...
parser.add_argument("-t", "--time", type=float, default=0.001)
...
def print_process_info():
    ...
    global loaded_model
    global syscall_pid_tid_dangerous_list
    global syscall_pid_tid_dangerous_prev_list
    global dangerous_check_number
    ...
    for k, v in dictionary_collect_print_data_syscall_info.items():
        ...
        input_data = [[v['syscall_total_count'], v['syscall_argument_similar'], v['syscall_kind_similar'], v['syscall_kind'], v['syscall_top1'],v['syscall_top2'],v['syscall_top3'],v['syscall_top4']
                    ,v['syscall_top5'],v['syscall_top6']]]
        prediction = loaded_model.predict(input_data)
        
        if prediction == 1:
            syscall_pid_tid_dangerous_list[v['pid']] = v['process_name']
            print('dangerous',write_data)

    for pid, syscall_pid_tid_dangerous_prev_number in syscall_pid_tid_dangerous_prev_list.items():
        syscall_pid_tid_dangerous_prev_list[pid][1] = 0
    for pid, process_name in syscall_pid_tid_dangerous_list.items():
        if syscall_pid_tid_dangerous_prev_list.get(pid) == None:
            syscall_pid_tid_dangerous_prev_list[pid] = [0,0]
        syscall_pid_tid_dangerous_prev_list[pid][0] += 1
        syscall_pid_tid_dangerous_prev_list[pid][1] = 1
        if syscall_pid_tid_dangerous_prev_list[pid][0] > dangerous_check_number:
            write_data = [print_type,pid,process_name,'real dangerous']
            print_line_count += 1
            print(write_data)

    delete_list = []
    for pid, syscall_pid_tid_dangerous_prev_number in syscall_pid_tid_dangerous_prev_list.items():
        if syscall_pid_tid_dangerous_prev_list[pid][1] == 0:
            syscall_pid_tid_dangerous_prev_list[pid][0] = 0
        if syscall_pid_tid_dangerous_prev_list[pid][0] < 1:
            delete_list.append(pid)
    for pid in delete_list:
        del(syscall_pid_tid_dangerous_prev_list[pid])

    syscall_pid_tid_dangerous_list = {}
...
dangerous_check_number = 2
syscall_pid_tid_dangerous_list = {}
syscall_pid_tid_dangerous_prev_list = {}
