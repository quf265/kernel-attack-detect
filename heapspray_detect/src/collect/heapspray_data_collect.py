    ...
    */
    ...
    if(val_thread_kmalloc_info)
    {
        if(val_thread_kmalloc_info->alloc_size == 0)
        {
            ...
            val_thread_kmalloc_info->cpu_id = cpu_id;
        }
        else
        {
...
TRACEPOINT_PROBE(kmem, kfree){
    ...
    if(val_thread_kmalloc_info)
    {
        ...
        cpu_id = val_thread_kmalloc_info->cpu_id;
    ...
    if args.label == 0:
        for k, v in dictionary_collect_print_data_syscall_info.items():
            ...
            if v['danger'] == 0:
                continue
            print_check = 1
            if v['process_name'] == args.name:
                write_data = [print_type,v['tgid'],v['tid'],v['process_name'],v['kmalloc_total_count'], v['kmalloc_count'], v['kfree_count'], v['kmalloc_kind'], v['kmalloc_top_entropy'], v['kmalloc_top_gini'],1]
                writer.writerow(write_data)
                print_line_count += 1
                if args.print:
                    print(write_data)
            else:
                write_data = [print_type,v['tgid'],v['tid'],v['process_name'],v['kmalloc_total_count'], v['kmalloc_count'], v['kfree_count'], v['kmalloc_kind'], v['kmalloc_top_entropy'],v['kmalloc_top_gini'],args.label]
                writer.writerow(write_data)
                print_line_count += 1
                if args.print:
                    print(write_data)
        ...
        for k, v in dictionary_collect_print_data_syscall_info.items():
            if v['kmalloc_total_count'] == 0:
                ...
                continue
            print_check = 1
            write_data = [print_type,v['tgid'],v['tid'],v['process_name'],v['kmalloc_total_count'], v['kmalloc_count'], v['kfree_count'], v['kmalloc_kind'], v['kmalloc_top_entropy'],v['kmalloc_top_gini']]
            writer.writerow(write_data)
            print_line_count += 1
            if args.print:
                print(write_data)
